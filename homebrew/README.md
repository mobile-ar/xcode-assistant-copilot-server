# Homebrew Tap for xcode-assistant-copilot-sever

This directory contains the Homebrew formula for [xcode-assistant-copilot-sever](https://github.com/mobile-ar/xcode-assistant-copilot-sever).

## Setting Up the Tap Repository

To distribute via Homebrew, create a separate GitHub repository named **`homebrew-xcode-assistant-copilot-sever`** under the `mobile-ar` organization.

### 1. Create the tap repository

```sh
gh repo create mobile-ar/homebrew-xcode-assistant-copilot-sever --public --description "Homebrew tap for xcode-assistant-copilot-sever"
```

### 2. Initialize the tap repository

```sh
git clone git@github.com:mobile-ar/homebrew-xcode-assistant-copilot-sever.git
cd homebrew-xcode-assistant-copilot-sever
mkdir -p Formula
```

### 3. Copy the formula

Copy `Formula/xcode-assistant-copilot-sever.rb` from this directory into the tap repository:

```sh
cp /path/to/xcode-assistant-copilot-sever/homebrew/Formula/xcode-assistant-copilot-sever.rb Formula/
```
...
git commit -m "Add xcode-assistant-copilot-sever formula template"
...
```sh
cd /path/to/xcode-assistant-copilot-sever
git tag v1.0.0
...
5. **Clones the tap repository** (`mobile-ar/homebrew-xcode-assistant-copilot-sever`).
...
6. Under **Repository access**, select **Only select repositories**, then pick **`homebrew-xcode-assistant-copilot-sever`** from the dropdown. This ensures the token cannot access any other repository.
...
1. Go to the main project repository: [github.com/mobile-ar/xcode-assistant-copilot-sever](https://github.com/mobile-ar/xcode-assistant-copilot-sever).
...
3. Once it completes, verify:
   ...
   - The tap repository's `Formula/xcode-assistant-copilot-sever.rb` has the correct URL and SHA256.
